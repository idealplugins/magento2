...
class Bancontact extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    public function setupPayment($bankId = false)
    {
        ...
        $testMode = (bool) $this->_scopeConfig->getValue('payment/bancontact/testmode');
        
        $digiCore = new TargetPayCore(
            $this->tpMethod,
            $this->_scopeConfig->getValue('payment/bancontact/rtlo'),
            $language,
            $testMode
        );
...
//         $digiCore->setBankId($bankId);
        $digiCore->setReturnUrl(
            $this->urlBuilder->getUrl('bancontact/bancontact/return', ['_secure' => true, 'order_id' => $orderId])
        );
        $digiCore->setReportUrl(
            $this->urlBuilder->getUrl('bancontact/bancontact/report', ['_secure' => true, 'order_id' => $orderId])
        );
