...
class Ideal extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    public function setupPayment($bankId = false)
    {
        ...
        if ($order->getGrandTotal() < $this->minAmount) {
            throw new \Magento\Checkout\Exception(
                __('Het totaalbedrag is lager dan het minimum van ' . $this->minAmount . ' euro voor ' . Ideal::METHOD_CODE)
                );
        }
        
        if ($order->getGrandTotal() > $this->maxAmount ) {
            throw new \Magento\Checkout\Exception(
                __('Het totaalbedrag is hoger dan het maximum van ' . $this->maxAmount . ' euro voor ' . Ideal::METHOD_CODE)
                );
        }

        $orderId = $order->getRealOrderId();
        $language = ($this->localeResolver->getLocale() == 'nl_NL') ? "nl" : "en";
        $testMode = (bool) $this->_scopeConfig->getValue('payment/ideal/testmode');
    ...
    /**
     * Retrieve available payment banks
     *
     * @return array
     */
